...
import { Eip4824RegistrationSummoner } from '../src/types/Eip4824RegistrationSummoner'
...
const _addresses = {
    ...
    regSummoner: '0x3d6f3410fb45d14f01320f2fb2e75f6504f9d777',
}
...
task('ragequit', 'Ragequit shares and/or loot')
    ...
    .addParam('tokens', 'the token addresses (array) (escape quotes) (no spaces) ex [\\"0x123...\\"]')
    .setAction(async (taskArgs, hre) => {
        const Baal = await hre.ethers.getContractFactory('Baal')
        const baal = (await Baal.attach(taskArgs.dao)) as Baal
...
task('tributeprop', 'Approve token and make a tribute proposal')
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const encoded = await minion.encodeTributeProposal(taskArgs.dao, taskArgs.shares, taskArgs.loot, address, countBefore + 1, taskArgs.minion)
...
task('summon', 'Summons a new DAO')
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const getBaalParams = async function (
            ...
            poster: Poster,
            regSummoner: Eip4824RegistrationSummoner,
            daoUri: string,
            config: {
            ...
            const register = await regSummoner.interface.encodeFunctionData('summonRegistration', [daoUri])
            const posterFromBaal = await baal.interface.encodeFunctionData('executeAsBaal', [poster.address, 0, postMetaData])

            const initalizationActions = [setAdminConfig, setGovernanceConfig, setShaman, mintShares, mintLoot, posterFromBaal, register]
        ...
        const regSummonerAddress = _addresses.regSummoner
        const RegSummoner = await hre.ethers.getContractFactory('EIP4824RegistrationSummoner')

        console.log('reg summoner address', regSummonerAddress)
        const regSummoner = (await RegSummoner.attach(regSummonerAddress)) as Eip4824RegistrationSummoner
        ...
        const sharesSingleton = (await SharesFactory.attach(taskArgs.sharestemplate)) as Shares
        const Baal = await hre.ethers.getContractFactory('Baal')
